...
func _draw() -> void:
	...
	#for i in range(1, int(Game5.cell_amount.x - 1)):
		#var from := Vector2(i * Game5.cell_size.x, Game5.cell_size.y)
		#var to := Vector2(from.x, Game5.grid_size.y - Game5.cell_size.y)
		#draw_line(from, to, Colors.GREYFORLINING, 2.0)
	#for i in range(1, int(Game5.cell_amount.y - 1)):
		#var from := Vector2(Game5.cell_size.x, i * Game5.cell_size.y)
		#var to := Vector2(Game5.grid_size.x - Game5.cell_size.x, from.y)
		#draw_line(from, to, Colors.GREYFORLINING, 2.0)

	# Draw the outline for the selected tile
	if selected_tile_pos != null:
		var outline_rect = Rect2(selected_tile_pos.x, selected_tile_pos.y, Game5.cell_size.x, Game5.cell_size.y)
...
func apply_gravity():
	for x in range(1, int(Game5.cell_amount.x - 1)):
		var empty_cells = []
		for y in range(int(Game5.cell_amount.y - 2), 0, -1):  # Iterate from bottom to top
			var grid_pos = Vector2(x, y)
			if grid_data[y][x] == -1:  # If cell is empty
				empty_cells.append(grid_pos)
			elif not empty_cells.is_empty():  # If cell is not empty and there are empty cells below
				var new_pos = empty_cells.pop_front()
				# Move the Pokemon instance
				var pokemon = get_pokemon_at_pos(grid_pos)
				if is_instance_valid(pokemon):
					pokemon.position = Vector2(new_pos.x * Game5.cell_size.x + Game5.cell_size.x / 2, new_pos.y * Game5.cell_size.y + Game5.cell_size.y / 2)
					# Update pokemon_map
					pokemon_map.erase(grid_pos)
					pokemon_map[new_pos] = pokemon
					# Update grid_data
					grid_data[int(new_pos.y)][int(new_pos.x)] = grid_data[int(grid_pos.y)][int(grid_pos.x)]
					grid_data[int(grid_pos.y)][int(grid_pos.x)] = -1
					empty_cells.append(grid_pos)
...
func generate_grid():
	...
	grid_data = []
	for y in range(int(Game5.cell_amount.y)):
		var row = []
		for x in range(int(Game5.cell_amount.x)):
			if x == 0 or x == Game5.cell_amount.x - 1 or y == 0 or y == Game5.cell_amount.y - 1:
	...
	var inner_width = int(Game5.cell_amount.x - 2)
	var inner_height = int(Game5.cell_amount.y - 2)
	var inner_grid_data = generate_grid_data(inner_width, inner_height, Game5.pokemon_data)
	...
	for y in range(grid_data.size()):
		for x in range(grid_data[y].size()):
			...
			if pokemon_id != -1:  # -1 represents an empty cell
				var position = Vector2(x * Game5.cell_size.x + Game5.cell_size.x / 2, y * Game5.cell_size.y + Game5.cell_size.y / 2)
...
func check_game_state():
	...
	for y1 in range(1, int(Game5.cell_amount.y - 1)):
		for x1 in range(1, int(Game5.cell_amount.x - 1)):
			if grid_data[y1][x1] != -1:  # If cell is not empty
				for y2 in range(1, int(Game5.cell_amount.y - 1)):
					for x2 in range(1, int(Game5.cell_amount.x - 1)):
						if y1 == y2 and x1 == x2:
							continue
...
func shuffle_grid():
	...
	for y in range(1, int(Game5.cell_amount.y - 1)):
		for x in range(1, int(Game5.cell_amount.x - 1)):
			if grid_data[y][x] != -1:
				var pos = Vector2(x, y)
	...
	for i in range(len(remaining_pokemon_ids)):
		...
		var position = Vector2(pos.x * Game5.cell_size.x + Game5.cell_size.x / 2, pos.y * Game5.cell_size.y + Game5.cell_size.y / 2)
		spawn_pokemon(position, pokemon_id)
...
func spawn_pokemon(position, pokemon_id):
	var pokemon_scene = Game5.pokemon_data[pokemon_id].scene
	if is_instance_valid(pokemon_scene):
		...
		if is_instance_valid(pokemon_instance):
			...
			pokemon_instance.name = "Pokemon_" + Game5.pokemon_data[pokemon_id].name
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			if is_instance_valid(clicked_pokemon):
				...
				var grid_pos = clicked_pokemon.position - Game5.cell_size / 2
				grid_pos = Vector2(int(grid_pos.x / Game5.cell_size.x), int(grid_pos.y / Game5.cell_size.y))

				if selected_tile == null:
					# Select the first tile
					selected_tile = clicked_pokemon
					selected_tile_pos = grid_pos * Game5.cell_size # Store the position for _draw()
...
func get_grid_pos(position):
	var grid_pos = position - Game5.cell_size / 2
	return Vector2(int(grid_pos.x / Game5.cell_size.x), int(grid_pos.y / Game5.cell_size.y))
...
func is_valid_position(pos):
	...
	return pos.x >= 0 and pos.x < Game5.cell_amount.x and pos.y >= 0 and pos.y < Game5.cell_amount.y
